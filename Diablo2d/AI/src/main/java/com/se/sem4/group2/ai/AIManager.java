    ...
    private void updateCellHash(WorldMap worldMap) {
        for (int x = worldMap.getxMin(); x < worldMap.getxMax(); x++) {
            ...
            for (int y = worldMap.getyMin(); y < worldMap.getyMax(); y++) {
                ...
                // A divisor is introduced to reduce the workload. 
                // This reduces the precision of the pathfinding. 
                // Set to 1 to keep per pixel precision pathfinding 
        ...
        // Update the bounds to make sure we don't process the same cell twice
